...
class WechatAuth {
    ...
    public function jsTicket(){
        $param = array('type'=>'jsapi');
        $name = "ticket/getticket";

        $arr = $this->api($name,'',"GET",$param);
        if($arr['errcode']>0){
            E($arr['errmsg']);
        }
        return $arr;
    ...
    /**
     * @param $toOpenid 接收模版消息的openid
     * @param $templateId 模版消息ID
     * @param $url 链接
     * @param $data 模版消息参数
     */
    public function sendTemplate($toOpenid, $templateId,$url, $data){
        $name="message/template/send";
        $post = array('touser'=>$toOpenid,'template_id'=>$templateId,'url'=>$url,'data'=>$data);
        return $this->api($name,$post);
    }
