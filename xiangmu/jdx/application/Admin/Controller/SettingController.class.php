...
class SettingController extends AdminbaseController{
	...
	//展示商城设置数据
	function mall_setting()
	{
		$option = $this->options_model->where("option_name='mall_setting'")->find();
		$this->assign('option_id',$option['option_id']);
		$this->assign('mall_setting',json_decode($option['option_value'],true));
		$this->display();
	}

	//商城设置处理
	function mall_setting_post()
	{
		if(IS_POST){
			if(isset($_POST['option_id'])){
				$data['option_id']=intval($_POST['option_id']);
			}
			$data['option_name']="mall_setting";
			$data['option_value'] = json_encode($_POST['mall']);
			$rst = $this->options_model->save($data);
			if($rst !==false){
				D('Log')->addLog('修改商城设置');
				sp_clear_cache();
				$this->success("保存成功！");
