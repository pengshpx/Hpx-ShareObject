...
function change_images(uploadid, returnid) {
    ...
    $.each(contents, function(i, n) {
        ...
        str += "<li id='image" + ids + "'>" +
            "<input readonly title='双击查看' type='text' name='" + returnid + "_url[]' value='" + n + "' style='width:310px;' ondblclick='image_priview(this.value);' class='input image-url-input'>" +
            //" <input readonly type='text' name='" + returnid + "_alt[]' value='" + filename + "' style='width:160px;' class='input image-alt-input' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\"> " +
            "<a href=\"javascript:flashupload('replace_albums_images', '图片替换','image"+ids+"',replace_image,'1,gif|jpg|jpeg|png|bmp,0','','','')\">替换</a> " +
...
function replace_image2(uploadid, returnid) {
    var d = uploadid.iframe.contentWindow;
    var in_content = d.$("#att-status").html().substring(1);
    var in_filename = d.$("#att-name").html().substring(1);
    var contents = in_content.split('|');
